//Originally from https://www.thingiverse.com/thing:1092611
...
module arc(radius, thick, angle){
	intersection(){
		...
		difference(){
			circle(radius+thick,$fn=128);
			circle(radius,$fn=128);
		}
	}
}


module wedge(radius, thick, angle, height){
  linear_extrude(height) arc(radius, thick, angle);
}
