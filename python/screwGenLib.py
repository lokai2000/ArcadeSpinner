...
def buildScrew(screw,verbose=0):
  ...
  #Number of transition teeth at each end
  T = 0
  ...
  for tooth in range(N+2*T):
    ...
    for layer in range(fNT):
      ...
      if (tooth<T) :
        dS = (layer + tooth*fNT)*sStep
        dT = 2.0*dR - dS*dR
        #dT = dR
      elif ( tooth >= T+N ):
        dS = 1.0-(layer + (tooth-(T+N))*fNT)*sStep
        dT = 2.0*dR - dS*dR
      else:
        dS = 1.0
        dT = dR
      ...
      for outer in range(fNR):
        ...
        #Fixme
        R = R1 + dT + dS*dR*math.sin(layer*phaseStep-offset)
